...
`;
  ...
  dashScript.onload = () => {
    // --- PRIMA COSA: riferimenti a video, wrapper e controlli ---
    ...
    // Helper per formattare i timecode
    ...
    // Funzione per auto-hide dei controlli e del cursore
    ...
    // Custom subtitles fade-in/fade-out
    ...
    progress.addEventListener('mousemove', e => {
      ...
      const x = pct * (rect.width - thumb.offsetWidth) + thumb.offsetWidth / 2;
      ...
      const minX = 24;
      const maxX = wrapRect.width - previewContainer.offsetWidth - 24;
    ...
    volume.addEventListener('input', () => {
      ...
      document.getElementById('volume-icon-player-video-il-silenzio-della-natura-desktop').src = iconURL;
      document.getElementById('volume-icon-player-video-il-silenzio-della-natura-desktop').alt = altText;
    });
    volume.dispatchEvent(new Event('input'));
    document.getElementById('volume-icon-player-video-il-silenzio-della-natura-desktop')
    ...
    // --- CLICK SUL VIDEO PER PLAY/PAUSE ---
    ...
    subsItems.forEach(item => {
      ...
      item.addEventListener('click', () => {
        ...
        // toggle sul main video
        for (let i = 0; i < video.textTracks.length; i++) {
          video.textTracks[i].mode = (i === val) ? 'hidden' : 'disabled';
        }
        // toggle sul preview
        for (let i = 0; i < previewVideo.textTracks.length; i++) {
          previewVideo.textTracks[i].mode = (i === val) ? 'hidden' : 'disabled';
        }
        // pulisci o mostra
    ...
    ['click','mousemove','keydown','wheel','touchstart','pointermove'].forEach(evt => {
      ...
      document.addEventListener(evt, resetHideControls, { passive: true });
    });
    // inizializza stato auto-hide
    resetHideControls();
  };
