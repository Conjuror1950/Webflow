...
`;
overlay.innerHTML = `
  <div class="lock-message" style="display:flex;flex-direction:column;align-items:center;gap:0.5rem">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 23.9668 19.5986" style="width:2.5rem;height:2.5rem;color:black;opacity:0.95">
      <g>
    ...
    <p style="font-family:'SF Pro Display',-apple-system,sans-serif;font-size:1rem;font-weight:500;color:black;margin:0;letter-spacing:0.02em">Ruotare il dispositivo</p>
  </div>
`;
document.body.appendChild(overlay);

// === Funzione di toggle istantanea ===
const checkOverlay = () => {
  const isMobileLandscape = window.matchMedia("(orientation: landscape) and (max-width: 767px)").matches;
  const isFullscreen = !!(document.fullscreenElement || document.webkitFullscreenElement || document.mozFullScreenElement || document.msFullscreenElement);
  overlay.style.opacity = isMobileLandscape && !isFullscreen ? '1' : '0';
  overlay.style.pointerEvents = isMobileLandscape && !isFullscreen ? 'all' : 'none';
};

// === Event listener immediati ===
window.addEventListener('orientationchange', checkOverlay, { passive: true });
window.addEventListener('resize', checkOverlay, { passive: true });
document.addEventListener('fullscreenchange', checkOverlay);
document.addEventListener('webkitfullscreenchange', checkOverlay);
document.addEventListener('mozfullscreenchange', checkOverlay);
document.addEventListener('msfullscreenchange', checkOverlay);
